...
export async function onRequestGet({ request, env }) {
  ...
  const comments = (await env.DB.prepare(
    `SELECT id, post_id, author_persona_id, body, provider, needs_verification, created_at
     FROM comments WHERE post_id IN (${ph}) ORDER BY created_at ASC`
  ).bind(...ids).all()).results || [];

  const byPost = {};
  for (const c of comments) (byPost[c.post_id] ||= []).push({
    id: c.id, personaId: c.author_persona_id, body: c.body,
    provider: c.provider, needsVerification: !!c.needs_verification,
  });
